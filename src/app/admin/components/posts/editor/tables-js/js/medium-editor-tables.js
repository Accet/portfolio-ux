	...
	(function() {
		...
		Grid.prototype = {
				...
				[].forEach.call(
					...
					function(el) {
						var cell = {
							...
							active =
								this._currentCell && cell.row <= this._currentCell.row && cell.column <= this._currentCell.column;
			...
			_html: function() {
				var width = this.columns * COLUMN_WIDTH + BORDER_WIDTH * 2,
					...
					html =
						'<div class="medium-editor-table-builder-grid clearfix" style="width:' +
						width +
						'px;height:' +
						height +
						'px;">';
			...
			_cellsHTML: function() {
				...
				this._cells.map(function(cell) {
					html +=
						'<a href="#" class="medium-editor-table-builder-cell' +
		...
		MediumEditorTable = MediumEditor.extensions.form.extend({
			...
			show: function() {
				...
				if (
					range.startContainer.nodeName.toLowerCase() === 'td' ||
